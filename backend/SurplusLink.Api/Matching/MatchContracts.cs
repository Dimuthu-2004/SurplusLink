...
public sealed class MatchQuery : MatchPageQuery, IValidatableObject
{
    ...
    [Required, RegularExpression("(?i)^(score|distance|estimatedTransportCost|createdAt)$")]
    ...
    {
        if (Status is not null && !Enum.GetNames<MatchStatus>().Contains(Status, StringComparer.OrdinalIgnoreCase))
            yield return new("Status must be a named match status.", [nameof(Status)]);
    }
}
