...
const emptyRegistration: RegistrationForm = {
  fullName: '', email: '', phoneNumber: '', businessName: '', address: '', password: '', confirmPassword: '', roles: [],
};
...
export function LoginPage() {
  ...
  async function submitRegistration(event: FormEvent<HTMLFormElement>) {
    ...
    if (await register(request)) navigate(destination(), { replace: true });
  }
...
function registrationError(form: RegistrationForm) { if (!form.fullName.trim()) return 'Enter your full name.'; if (!isEmail(form.email.trim())) return 'Enter a valid email address.'; if (!/^\+?[0-9][0-9 ()-]{6,24}$/.test(form.phoneNumber.trim())) return 'Enter a valid phone number.'; if (!form.address.trim()) return 'Enter your address.'; if (form.password.length < 8) return 'Your password must be at least 8 characters.'; if (form.password !== form.confirmPassword) return 'Passwords do not match.'; if (!form.roles.length) return 'Choose how you want to use SurplusLink.'; return null; }
