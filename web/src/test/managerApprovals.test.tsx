...
function api(): ManagerWorkflowsApi { return { transactions: vi.fn().mockResolvedValue({ items: [], page: 1, pageSize: 10, total: 0, totalPages: 0 }), completeTransaction: vi.fn(), transactionHistory: vi.fn(), list: vi.fn(), get: vi.fn().mockResolvedValue(workflow), approve: vi.fn().mockResolvedValue({ ...workflow, status: 'APPROVED' }), reject: vi.fn().mockResolvedValue({ ...workflow, status: 'REJECTED' }), revise: vi.fn().mockResolvedValue({ ...workflow, status: 'REVISION_REQUESTED' }), transactionAnalytics: vi.fn().mockResolvedValue({ pendingApprovalCount: 1, approvedCount: 2, rejectedCount: 0, reservedQuantity: 12, completionCount: 0, completedValue: 0, completionRate: null }) }; }
...
describe('manager workflow decisions', () => {
  it('shows the actual reserved quantity and completes an approved transfer', async () => {
    const client = api();
    ...
    await screen.findByRole('button', { name: 'Complete transfer' });
    expect(client.transactions).toHaveBeenCalledWith('match-1', 1);
    expect(screen.getAllByText('400')).toHaveLength(2);
    await userEvent.click(screen.getByRole('button', { name: 'Complete transfer' }));
    await waitFor(() => expect(client.completeTransaction).toHaveBeenCalledWith('transaction-1'));
  });
